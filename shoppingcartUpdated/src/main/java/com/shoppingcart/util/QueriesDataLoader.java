...
import com.shoppingcart.util.SessionFactoryBuilder;
import com.shoppingcart.util.Tasks;
import com.shoppingcart.bean.QueryColumnInfo;
import com.shoppingcart.bean.QueryInfo;
import com.shoppingcart.bean.QueryTableInfo;
import com.shoppingcart.util.CommonUtil;
import com.shoppingcart.util.layout.QueriesDataLoaderUtil;
public class QueriesDataLoader
{
	...
	public static int executeQueriesScript(String databaseName)throws Exception
	{
		...
		{							
			...
			isQueryAdded = addUserCartQueryInfo(organisationSession);
			if(isQueryAdded ==0)
			{
				System.err.println("UserCart query info could not be added.");
				if (tx != null)
					tx.rollback();
				return 0;	
			}			
			isQueryAdded = addCartItemQueryInfo(organisationSession);
			if(isQueryAdded ==0)
			{
				System.err.println("CartItem query info could not be added.");
				if (tx != null)
					tx.rollback();
				return 0;	
			}			
			isQueryAdded = addProductsQueryInfo(organisationSession);
			if(isQueryAdded ==0)
			{
				System.err.println("Products query info could not be added.");
				if (tx != null)
					tx.rollback();
				return 0;	
			}			
			isQueryAdded = addOrdersQueryInfo(organisationSession);
			if(isQueryAdded ==0)
			{
				System.err.println("Orders query info could not be added.");
				if (tx != null)
					tx.rollback();
				return 0;	
			}			
			isQueryAdded = addOrderItemQueryInfo(organisationSession);
			if(isQueryAdded ==0)
			{
				System.err.println("OrderItem query info could not be added.");
				if (tx != null)
					tx.rollback();
				return 0;	
			}			
	...
	private static int addUserCartQueryInfo(Session organisationSession)throws Exception
	{
		QueryTableInfo queryTableInfo;
		QueryInfo queryInfo;
		QueryColumnInfo queryColumnInfo;
		...
		QueriesDataLoaderUtil queryLoader = new QueriesDataLoaderUtil();
		queryInfo = new QueryInfo();		
		queryInfo.setQueryCode("UserCart");
		queryInfo.setQueryName("UserCart");
		queryInfo.setUseNativeQuery("Yes");
		queryInfo.setQueryWhereClause(" Where 2>1 ");
		queryInfo.setGroupByClause("");
		queryInfo.setOrderByClause("");
		queryInfo.setLimitClause("");
				queryTableInfo = new QueryTableInfo();
		queryTableInfo.setQueryTableName("UserCart");
		queryTableInfo.setQueryTableAlias("uc");	
		queryTablesList.add(queryTableInfo);

				queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("uc.userCartId");
		queryColumnInfo.setQueryColumnAlias("uc_userCartId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("uc.cartCreationTime");
		queryColumnInfo.setQueryColumnAlias("uc_cartCreationTime");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("uc.cartCheckoutTime");
		queryColumnInfo.setQueryColumnAlias("uc_cartCheckoutTime");
		queryColumnsList.add(queryColumnInfo);
	...
	private static int addCartItemQueryInfo(Session organisationSession)throws Exception
	{
		QueryTableInfo queryTableInfo;
		QueryInfo queryInfo;
		QueryColumnInfo queryColumnInfo;
		...
		QueriesDataLoaderUtil queryLoader = new QueriesDataLoaderUtil();
		queryInfo = new QueryInfo();		
		queryInfo.setQueryCode("CartItem");
		queryInfo.setQueryName("CartItem");
		queryInfo.setUseNativeQuery("Yes");
		queryInfo.setQueryWhereClause(" Where 2>1 ");
		queryInfo.setGroupByClause("");
		queryInfo.setOrderByClause("");
		queryInfo.setLimitClause("");
				queryTableInfo = new QueryTableInfo();
		queryTableInfo.setQueryTableName("CartItem");
		queryTableInfo.setQueryTableAlias("ci");	
		queryTablesList.add(queryTableInfo);

				queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("ci.cartItemId");
		queryColumnInfo.setQueryColumnAlias("ci_cartItemId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("ci.userCartId");
		queryColumnInfo.setQueryColumnAlias("ci_userCartId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("ci.productId");
		queryColumnInfo.setQueryColumnAlias("ci_productId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("ci.productQuantity");
		queryColumnInfo.setQueryColumnAlias("ci_productQuantity");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("ci.productUnitPrice");
		queryColumnInfo.setQueryColumnAlias("ci_productUnitPrice");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("ci.subTotalAmount");
		queryColumnInfo.setQueryColumnAlias("ci_subTotalAmount");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("ci.grandTotal");
		queryColumnInfo.setQueryColumnAlias("ci_grandTotal");
		queryColumnsList.add(queryColumnInfo);
	...
	private static int addProductsQueryInfo(Session organisationSession)throws Exception
	{
		QueryTableInfo queryTableInfo;
		QueryInfo queryInfo;
		QueryColumnInfo queryColumnInfo;
		...
		QueriesDataLoaderUtil queryLoader = new QueriesDataLoaderUtil();
		queryInfo = new QueryInfo();		
		queryInfo.setQueryCode("Products");
		queryInfo.setQueryName("Products");
		queryInfo.setUseNativeQuery("Yes");
		queryInfo.setQueryWhereClause(" Where 2>1 ");
		queryInfo.setGroupByClause("");
		queryInfo.setOrderByClause("");
		queryInfo.setLimitClause("");
				queryTableInfo = new QueryTableInfo();
		queryTableInfo.setQueryTableName("Products");
		queryTableInfo.setQueryTableAlias("p");	
		queryTablesList.add(queryTableInfo);

				queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("p.productsId");
		queryColumnInfo.setQueryColumnAlias("p_productsId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("p.productName");
		queryColumnInfo.setQueryColumnAlias("p_productName");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("p.productDescription");
		queryColumnInfo.setQueryColumnAlias("p_productDescription");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("p.productPrice");
		queryColumnInfo.setQueryColumnAlias("p_productPrice");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("p.productUnitType");
		queryColumnInfo.setQueryColumnAlias("p_productUnitType");
		queryColumnsList.add(queryColumnInfo);
	...
	private static int addOrdersQueryInfo(Session organisationSession)throws Exception
	{
		QueryTableInfo queryTableInfo;
		QueryInfo queryInfo;
		QueryColumnInfo queryColumnInfo;
		...
		QueriesDataLoaderUtil queryLoader = new QueriesDataLoaderUtil();
		queryInfo = new QueryInfo();		
		queryInfo.setQueryCode("Orders");
		queryInfo.setQueryName("Orders");
		queryInfo.setUseNativeQuery("Yes");
		queryInfo.setQueryWhereClause(" Where 2>1 ");
		queryInfo.setGroupByClause("");
		queryInfo.setOrderByClause("");
		queryInfo.setLimitClause("");
				queryTableInfo = new QueryTableInfo();
		queryTableInfo.setQueryTableName("Orders");
		queryTableInfo.setQueryTableAlias("o");	
		queryTablesList.add(queryTableInfo);

				queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("o.ordersId");
		queryColumnInfo.setQueryColumnAlias("o_ordersId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("o.userCartId");
		queryColumnInfo.setQueryColumnAlias("o_userCartId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("o.orderNo");
		queryColumnInfo.setQueryColumnAlias("o_orderNo");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("o.orderDate");
		queryColumnInfo.setQueryColumnAlias("o_orderDate");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("o.orderAmount");
		queryColumnInfo.setQueryColumnAlias("o_orderAmount");
		queryColumnsList.add(queryColumnInfo);
	...
	private static int addOrderItemQueryInfo(Session organisationSession)throws Exception
	{
		QueryTableInfo queryTableInfo;
		QueryInfo queryInfo;
		QueryColumnInfo queryColumnInfo;
		...
		QueriesDataLoaderUtil queryLoader = new QueriesDataLoaderUtil();
		queryInfo = new QueryInfo();		
		queryInfo.setQueryCode("OrderItem");
		queryInfo.setQueryName("OrderItem");
		queryInfo.setUseNativeQuery("Yes");
		queryInfo.setQueryWhereClause(" Where 2>1 ");
		queryInfo.setGroupByClause("");
		queryInfo.setOrderByClause("");
		queryInfo.setLimitClause("");
				queryTableInfo = new QueryTableInfo();
		queryTableInfo.setQueryTableName("OrderItem");
		queryTableInfo.setQueryTableAlias("oi");	
		queryTablesList.add(queryTableInfo);

				queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("oi.orderItemId");
		queryColumnInfo.setQueryColumnAlias("oi_orderItemId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("oi.orderId");
		queryColumnInfo.setQueryColumnAlias("oi_orderId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("oi.productId");
		queryColumnInfo.setQueryColumnAlias("oi_productId");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("oi.productQuantity");
		queryColumnInfo.setQueryColumnAlias("oi_productQuantity");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("oi.productUnitPrice");
		queryColumnInfo.setQueryColumnAlias("oi_productUnitPrice");
		queryColumnsList.add(queryColumnInfo);
		queryColumnInfo = new QueryColumnInfo();
		queryColumnInfo.setQueryColumnText("oi.subTotalAmt");
		queryColumnInfo.setQueryColumnAlias("oi_subTotalAmt");
		queryColumnsList.add(queryColumnInfo);
