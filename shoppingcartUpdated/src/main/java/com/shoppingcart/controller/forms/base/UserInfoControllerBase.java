...
//
import com.shoppingcart.bean.EmployeeRoles;
import com.shoppingcart.controller.forms.impl.EmployeeRolesControllerImpl;

import com.shoppingcart.bean.UserInfo;
import com.shoppingcart.controller.forms.base.UserInfoLabel;
...
public abstract class UserInfoControllerBase extends BusinessRulesController
{
	...
	{
		com.shoppingcart.bean.UserInfo entityBean = (UserInfo)entityObject;
		...
		if(userInfoListObject!=null && userInfoListObject.has("success") && userInfoListObject.get("success").getAsInt()==1)
		{
			...
			for (int i =0; i< userInfoList.size(); i++)
			{
				...
				int userInfoId = userInfoDataObject.get("userInfoId").getAsInt();
								com.shoppingcart.controller.forms.impl.EmployeeRolesControllerImpl employeeRolesImplObj = new com.shoppingcart.controller.forms.impl.EmployeeRolesControllerImpl(getDBSession(), getUserSessionInfo());
...
cell = row.createCell(headerCellCount++);
			...
			for (Iterator it = resultsList.iterator(); it.hasNext();)
			{
				...
				com.shoppingcart.bean.Organisations  organisationsUserOrgObj = null;
				if(organisationsUserOrgId > 0)
				{   
					cell.setCellValue(String.valueOf(organisationsUserOrgId));
					organisationsUserOrgObj = (com.shoppingcart.bean.Organisations) session.get(com.shoppingcart.bean.Organisations.class, organisationsUserOrgId);
				}   
		        				cell = row.createCell(dataCellCount++);
	...
	public int  areSomeRecordsUploaded(JsonObject previousRetryCountMap, JsonObject retryCountMap) throws Exception
	{
		...
		if(retryCount<previousRetryCount)
		{
			return 1;
	    		...
	    		com.shoppingcart.controller.forms.impl.EmployeeRolesControllerImpl employeeRolesImplObj = new com.shoppingcart.controller.forms.impl.EmployeeRolesControllerImpl(getDBSession());
	...
	
	{
		...
		try
		{
			for (int i =0; i < userInfoList.size(); i++)
			{
				...
				com.shoppingcart.controller.forms.impl.UserInfoControllerImpl userInfoImplObj = new com.shoppingcart.controller.forms.impl.UserInfoControllerImpl(session, getUserSessionInfo());				
			    ...
			    {
				    JsonArray employeeRolesList = userInfo.get("employeeRolesList").getAsJsonArray();
					com.shoppingcart.controller.forms.impl.EmployeeRolesControllerImpl employeeRolesImplObj = new com.shoppingcart.controller.forms.impl.EmployeeRolesControllerImpl(session, getUserSessionInfo());
	...
	public int getEntityInfoUpdatedWithLookupIds(Session session, JsonObject userInfo, JsonObject retValObject)
	{
		...
		int hasParamsForLookup = 0;
				hasParamsForLookup = com.shoppingcart.controller.forms.impl.OrganisationsControllerImpl.hasParamsForLookup(userInfo, "organisationsUserOrg");
		...
		{
			dataObject = com.shoppingcart.controller.forms.impl.OrganisationsControllerImpl.getQueryParamsForLookupSearch(userInfo, "organisationsUserOrg");
			...
			com.shoppingcart.controller.forms.impl.OrganisationsControllerImpl organisationsUserOrgImplObj = new com.shoppingcart.controller.forms.impl.OrganisationsControllerImpl(session);
