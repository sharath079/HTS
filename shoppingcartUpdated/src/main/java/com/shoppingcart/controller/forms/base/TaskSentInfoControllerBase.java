...
import com.shoppingcart.bean.TaskSentInfo;
import com.shoppingcart.controller.forms.base.TaskSentInfoLabel;
...
public abstract class TaskSentInfoControllerBase extends BusinessRulesController
{
	...
	{
		com.shoppingcart.bean.TaskSentInfo entityBean = (TaskSentInfo)entityObject;
...
//			executeAction(taskSentInfo, "TaskSentInfoBean", currentStatus);
	...
	
	{
		...
		try
		{
			...
			for (Iterator it = resultsList.iterator(); it.hasNext();)
			{
				...
				com.shoppingcart.bean.TaskInfo  taskInfoObj = null;
				if(taskInfoId > 0)
				{   
					cell.setCellValue(String.valueOf(taskInfoId));
					taskInfoObj = (com.shoppingcart.bean.TaskInfo) session.get(com.shoppingcart.bean.TaskInfo.class, taskInfoId);
				}   
	...
	
	{
		...
		try
		{
			for (int i =0; i < taskSentInfoList.size(); i++)
			{
				...
				com.shoppingcart.controller.forms.impl.TaskSentInfoControllerImpl taskSentInfoImplObj = new com.shoppingcart.controller.forms.impl.TaskSentInfoControllerImpl(session, getUserSessionInfo());				
	...
	public int getEntityInfoUpdatedWithLookupIds(Session session, JsonObject taskSentInfo, JsonObject retValObject)
	{
		...
		int hasParamsForLookup = 0;
				hasParamsForLookup = com.shoppingcart.controller.forms.impl.TaskInfoControllerImpl.hasParamsForLookup(taskSentInfo, "taskInfo");
		...
		{
			dataObject = com.shoppingcart.controller.forms.impl.TaskInfoControllerImpl.getQueryParamsForLookupSearch(taskSentInfo, "taskInfo");
