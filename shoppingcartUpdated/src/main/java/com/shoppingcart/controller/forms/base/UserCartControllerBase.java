...
import com.shoppingcart.controller.forms.base.UserCartLabel;
 ...
 *
 */
public abstract class UserCartControllerBase extends BusinessRulesController
{
	/*
	...
	protected UserCartLabel UserCartLabelLocal = new UserCartLabel();
	protected UserCart UserCartLocal = new UserCart();
	...
	public UserCartControllerBase(Session session)
	{
		super(session);
		userSessionInfo.addProperty("userId", -1);
		userSessionInfo.addProperty("loginEntityType", "");
	}
	public UserCartControllerBase(Session session, JsonObject userLoggedInfo)
	{
		super(session);
	...
	{
		return ((UserCart)getManagedBean()).getVwTxnStatus();
	}
	...
	{
		((UserCart)getManagedBean()).setVwTxnStatus(sStatus);
	}
	...
	{
		iPKValue=((UserCart)obj).getUserCartId();
	}
    ...
    {
		return "UserCartLabelBean";
    }
	protected Class<UserCart> setBeanClass()
	{
		return UserCart.class;
	}
	protected Class<UserCartLabel> setBeanLabelClass()
	{
		return UserCartLabel.class;
	}
	protected UserCart getLocalManagedBean()
    {
		return (UserCart)getManagedBean();
    }
	...
	public void doBeforeTransactionApproved()
	{
		...
		/*UserCart userCart = (UserCart)getManagedBean();
		String areChangesEffected = userCart.getAreChangesEffected();
		...
		{
			userCart.setAreChangesEffected("YES");			
		}*/
	...
	public void doBeforeTransactionRolledback()
	{
		...
		{
			userCart.setAreChangesEffected("NO");			
		}*/
		int generatedRequestId = -1;
		if(getHasTransactionFailed()==true)
		{
	...
	{
		UserCart userCart = (UserCart)getManagedBean();
		String sCurrentStatus = userCart.getVwTxnStatus();
	...
	public void doAfterSelectOptionChanged(String attributeName)
	{
		...
		{
			addCustomResponse(getTransactionFailureMessage());
		}//doAfterSelectOptionChangedImpl(userCart, attributeName);
		setHasTransactionFailed(false);
	}
	public void doAfterUserCartLoaded(int obj, JsonObject initParamsInfo)
	{
		...
		if(initParamsInfo.has("cartCreationTime"))
		{
			String cartCreationTimeString = initParamsInfo.get("cartCreationTime").getAsString();
			if(cartCreationTimeString.length() > 0)
			{
				try
				{
					boolean isValidTime = CommonUtil.isValid12HoursFormatTimeWithSeconds(cartCreationTimeString);
					if(isValidTime)
					{
						String cartCreationTime = CommonUtil.getTimeFrom12HoursFormatWithSeconds(cartCreationTimeString);
						userCart.setCartCreationTime(Time.valueOf(cartCreationTime));
					}
				}
				catch (Exception e)
				{
					setTransactionFailureMessage("Your request could not be processed as enter valid CartCreationTime");
				}
			}
		}
		
		if(initParamsInfo.has("cartCheckoutTime"))
		{
			String cartCheckoutTimeString = initParamsInfo.get("cartCheckoutTime").getAsString();
			if(cartCheckoutTimeString.length() > 0)
			{
				try
				{
					boolean isValidTime = CommonUtil.isValid12HoursFormatTimeWithSeconds(cartCheckoutTimeString);
					if(isValidTime)
					{
						String cartCheckoutTime = CommonUtil.getTimeFrom12HoursFormatWithSeconds(cartCheckoutTimeString);
						userCart.setCartCheckoutTime(Time.valueOf(cartCheckoutTime));
					}
				}
				catch (Exception e)
				{
					setTransactionFailureMessage("Your request could not be processed as enter valid CartCheckoutTime");
				}
			}
		}
		...
		setHasTransactionFailed(false);
		doAfterEntityLoadedImpl(userCart, initParamsInfo);
		if(getHasTransactionFailed()==true)
		{
			addCustomResponse(getTransactionFailureMessage());
		}
	...
	public void doAfterLookupRowSelected(String attributeName, Integer attributeId)
	{
		...
		setHasTransactionFailed(false);
		doAfterLookupRowSelectedImpl(userCart, attributeName);
		if(getHasTransactionFailed()==true)
		{
			addCustomResponse(getTransactionFailureMessage());
		}
	...
	public void beforeDeleteTransaction(boolean clearSession)
	{
		...
		{JsonObject dataObject = new JsonObject();		if(isTransactionUpdatable()==false)
			...
			UserCart userCart = (UserCart)getPrivateManagedBean();
				...
				if (!isEntityAuditType(getManagedBeanName()))
				{
					...
					com.shoppingcart.bean.MessageQueue messageQueue = new com.shoppingcart.bean.MessageQueue();
						if (isExists(sCurrentLCNo))
						{
							...
							String sMsgCategory="";
							Integer iMqEntityId = mqservice.writeToQueue("CREATED", messageQueue.getMyBankBIC(), messageQueue.getTheirBankBIC(), messageQueue.getMsgFromChannel(), messageQueue.getMsgToChannel(), "UserCart", generateMGUID(), messageQueue.getMsgFormat(), "", sMsgGroupName, sMsgCategory, sMsgDirection, "", "", "", messageQueue.getMsgAppId(), messageQueue.getMsgServiceId(), sCurrentLCNo, sEntityId, (String)getAuthAmtCcy(), (BigDecimal)getAuthAmt());
						}
				}
			}
			catch (Exception e)
	...
	{
		debugCode("In getSearchParams() UserCartContollerBase");
		HashMap<String,Object> searchParams = new HashMap<String,Object>();
		/*searchBeanLocal = (UserCartSearch)getManagedBean("UserCartSearchBean");
		if (isExists(searchBeanLocal))
		{
						if (isExists(searchBeanLocal.getCartCreationTime()))
			{
				searchParams.put(UserCartLabelLocal.getcartCreationTimeFieldName(),searchBeanLocal.getCartCreationTime());
			}	
			if (isExists(searchBeanLocal.getCartCheckoutTime()))
			{
				searchParams.put(UserCartLabelLocal.getcartCheckoutTimeFieldName(),searchBeanLocal.getCartCheckoutTime());
			}	

			if (isExists(searchBeanLocal.getVwTxnStatus()))
			{
				searchParams.put(UserCartLabelLocal.getvwTxnStatusFieldName(),searchBeanLocal.getVwTxnStatus());
			}	
		}
		*/
		debugCode("Out getSearchParams() UserCartContollerBase");
        return searchParams;
	}
	public void setValues(Object sourceBean,Object targetBean,Boolean bSetAsManagedBean)
	{
		debugCode("In setValues UserCartContollerBase");
		targetBean = (UserCart)targetBean;((UserCart)targetBean).setUserCartId(((UserCart)sourceBean).getUserCartId());
				((UserCart)targetBean).setCartCreationTime(((UserCart)sourceBean).getCartCreationTime());
		((UserCart)targetBean).setCartCheckoutTime(((UserCart)sourceBean).getCartCheckoutTime());

		doAfterSetValues();
		debugCode("Out setValues UserCartContollerBase");
	}
	public Object getFieldValue(Object entityObject,String sFieldName)
	{
		com.shoppingcart.bean.UserCart entityBean = (UserCart)entityObject;
	 	if (sFieldName.equalsIgnoreCase("userCartId")) 
	 	{
			return entityBean.getUserCartId();
		}
	 	 	if (sFieldName.equalsIgnoreCase("CartCreationTime")) 
	 	{
			return entityBean.getCartCreationTime();
		}
	 	if (sFieldName.equalsIgnoreCase("CartCheckoutTime")) 
	 	{
			return entityBean.getCartCheckoutTime();
		}
	...
	{
		debugCode("In doEnrichSystemValues(String sAction) UserCartControllerBase");
		localManagedBean = getLocalManagedBean();
		String sActionBy = "AUTO";
		((UserCart) localManagedBean).setVwLastModifiedDate(new Date());
		((UserCart) localManagedBean).setVwLastModifiedTime(getCurrentTime());
		((UserCart) localManagedBean).setVwLastAction(sAction);
		if (isExists(sNextStatus)) 
		{
			((UserCart) localManagedBean).setVwTxnStatus(sNextStatus);
		}
		else if (sAction.matches(ACTION_CREATE)) 
		{
			((UserCart) localManagedBean).setVwTxnStatus("CREATED");
			((UserCart) localManagedBean).setIsRequestUnderProcesss(0);
		}
		else if (sAction.matches(ACTION_UPDATE)) 
		{
			//((UserCart) localManagedBean).setVwTxnStatus("MODIFIED");
		}
		if (isActionFromUI())
		{
			sActionBy = getLoggedInUser();
		}	
		((UserCart) localManagedBean).setVwModifiedBy(sActionBy);
		//doEnrichCustomLKValues();
		debugCode("Out doEnrichSystemValues(String sAction) UserCartControllerBase");
	}
	protected void doEnrichAuditValues(String sAction)
	{
		debugCode("In doEnrichAuditValues(String sAction) UserCartControllerBase");
		debugCode("Out doEnrichAuditValues(String sAction) UserCartControllerBase");
	}
	...
	{
		debugCode("In doValidations UserCartControllerBase");
		//NG: Important comment
		//managedBean = (UserCart) getManagedBean();
		localManagedBean = getLocalManagedBean();
		...
		doValidationsImpl();
		debugCode("Out doValidations UserCartControllerBase");
	}
	private void doAllowedDecimalValidation()
	{
		debugCode("In doAllowedDecimalValidation UserCartControllerBase");
		debugCode("Out doAllowedDecimalValidation UserCartControllerBase");
	}
	private void doAllowedValuesValidation()
	{
		debugCode("In doAllowedValuesValidation UserCartControllerBase");debugCode("Out doAllowedValuesValidation UserCartControllerBase");
	}
	private void doMandatoryValidation()
	{
		debugCode("In doMandatoryValidation UserCartControllerBase");
		debugCode("Out doMandatoryValidation UserCartControllerBase");
	}
	private void doLengthValidation()
	{
		debugCode("In doLengthValidation UserCartControllerBase");
				
		Date sFieldValue2 = ((UserCart) localManagedBean).getCartCreationTime();
		
		Date sFieldValue3 = ((UserCart) localManagedBean).getCartCheckoutTime();
		if (!isLengthAllowed(sFieldValue2,"20")) addMaxLengthResponse(UserCartLabelLocal.getcartCreationTimeFieldName(),"20");
		if (!isLengthAllowed(sFieldValue3,"20")) addMaxLengthResponse(UserCartLabelLocal.getcartCheckoutTimeFieldName(),"20");
debugCode("Out doLengthValidation UserCartControllerBase");
	}
	private void doDataTypeValidation()
	{
		debugCode("In doDataTypeValidation UserCartControllerBase");
		debugCode("Out doDataTypeValidation UserCartControllerBase");
	}
	private void doUniqueValidation()
	{
	 	debugCode("In doUniqueValidation UserCartContollerBase");
	...
	{
		debugCode("IN getLabel UserCartContollerBase");
		String sLabel = new UserCartLabel().getLabel(sResponseField); 
		debugCode("Out getLabel UserCartContollerBase");
		return sLabel;
	}	
	public JsonObject getEntityAttributeValue(JsonObject requestInfo) throws Exception
	{
		...
		try
		{
			int userCartId = requestInfo.get("objectId").getAsInt();
			JsonObject searchParams = new JsonObject();
			searchParams.addProperty("userCartId", userCartId);
			JsonObject responseData = retrieveUserCart(searchParams, new JsonObject());
			if(!responseData.has("success") || responseData.get("success").getAsInt() !=1)
			{
				dataObject.addProperty("alert", "'UserCart' could not be retrieved !!");			
				dataObject.addProperty("success", 0);			
				return dataObject;
			}
			String attributeName = requestInfo.get("attributeName").getAsString();
			JsonObject userCartDataObject = responseData.get("userCartDataObject").getAsJsonObject();
			dataObject.addProperty(attributeName, userCartDataObject.get(attributeName).getAsString());
	...
	public JsonObject retrieveUserCart(JsonObject requestParams, JsonObject paramsInfoObj) throws Exception
	{
		JsonObject dataObject = new JsonObject();
		Integer userCartId = -1;
		if(requestParams.has("userCartId"))
		{
			userCartId = requestParams.get("userCartId").getAsInt();
		}
		Map<String, String> paramsMap = new HashMap<String, String>();paramsMap.put("userCartId", String.valueOf(userCartId));
				String cartCreationTime = "";
		if(requestParams.has("cartCreationTime"))
		{
			paramsMap.put("cartCreationTime", requestParams.get("cartCreationTime").getAsString());
		}
		String cartCheckoutTime = "";
		if(requestParams.has("cartCheckoutTime"))
		{
			paramsMap.put("cartCheckoutTime", requestParams.get("cartCheckoutTime").getAsString());
		}

		JsonObject userCartListObject = retrieveUserCartList(paramsMap);
		if(userCartListObject!=null && userCartListObject.has("success") && userCartListObject.get("success").getAsInt()==1)
		{
			JsonArray userCartList = userCartListObject.get("userCartList").getAsJsonArray();
			if(userCartList.size()>0)
			{
				dataObject.add("userCartDataObject", userCartList.get(0).getAsJsonObject());
				dataObject.addProperty("success", 1);
				return dataObject;				
			}
		}
		dataObject.addProperty("alert", "Information of 'UserCart' could not be retrieved !!");			
		dataObject.addProperty("success", 0);			
		return dataObject;
	}
	public JsonObject getUserCart(Session session, JsonObject searchParams, String overrideWhereClause, String whereClause, String orderByClause)
	{
		JsonObject dataObject = new JsonObject();
		try
		{
			...
			Map<String, String> paramsMap = new HashMap<String, String>();
						String cartCreationTime = "";
			if(searchParams.has("cartCreationTime"))
			{
				paramsMap.put("cartCreationTime", searchParams.get("cartCreationTime").getAsString());
			}
			String cartCheckoutTime = "";
			if(searchParams.has("cartCheckoutTime"))
			{
				paramsMap.put("cartCheckoutTime", searchParams.get("cartCheckoutTime").getAsString());
			}
			...
			if(userCartListObject!=null && userCartListObject.has("success") && userCartListObject.get("success").getAsInt()==1)
			{
				...
				if(userCartList.size()>0)
				{
					dataObject.add("userCart", userCartList.get(0).getAsJsonObject());
					dataObject.addProperty("success", 1);
					return dataObject;				
				}
			}
	...
	public JsonObject getUserCartInJson(int userCartId)
	{
		JsonObject UserCartData = null;
		List<Integer> userCartIdsList = new ArrayList<>();
		userCartIdsList.add(userCartId);
		JsonArray userCartList = getUserCartListInJson(userCartIdsList);
		if(userCartList!=null && userCartList.size()>0)
		{
			UserCartData = userCartList.get(0).getAsJsonObject();
		}
		return UserCartData;
	}
	public JsonArray getUserCartListInJson(List<Integer> userCartIdsList)
	{
		Map<String, String> paramsMap = new HashMap<String, String>();
		JsonArray userCartObjectsList = new JsonArray();
		JsonObject userCartListObject = retrieveUserCartList(paramsMap, userCartIdsList);
		if(userCartListObject!=null && userCartListObject.has("success") && userCartListObject.get("success").getAsInt()==1)
		{
			JsonArray userCartList = userCartListObject.get("userCartList").getAsJsonArray();
			for (int i =0; i< userCartList.size(); i++)
			{
				JsonObject userCartDataObject = userCartList.get(i).getAsJsonObject();
				int userCartId = userCartDataObject.get("userCartId").getAsInt();
				
				userCartObjectsList.add(userCartDataObject);
			}
		}
		return userCartObjectsList;
	}
		
	public String getUploadedDataErrorMessage(Session session, JsonArray userCartList)
	{
		String errorMessage = "userCartList: \n";
		for (int i =0; i< userCartList.size(); i++)
		{
			JsonObject userCartDataObject = userCartList.get(i).getAsJsonObject();
			JsonObject userCart = userCartDataObject.get("dataObject").getAsJsonObject();if(!userCartDataObject.has("isSuccessfullyUploaded"))
			{
				errorMessage += "userCart could not be uploaded verify data \n"; 
			}
			else if(userCartDataObject.has("isSuccessfullyUploaded") 
					&& userCartDataObject.get("isSuccessfullyUploaded").getAsInt() == 0)
			{
				errorMessage += userCartDataObject.get("errorMessage").getAsString() +"\n"; 
			}
	...
	public String getLoginBasedWhereClause(java.util.Map<String, String> paramsMap)
	{
		...
		else if("UserCart".equalsIgnoreCase(loginEntityType))
		{
			loginBasedWhereClause = " AND userCartId = :userCartId ";
	...
	public void setLoginBasedWhereClauseParams(java.util.Map<String, String> paramsMap, Query query)
	{
		...
		{
			query.setParameter("userCartId", userId);
		}
		
	}
	public String getParentFilterClauseForUserCart(java.util.Map<String, String> paramsMap)
	{
	...
	public void setParentFilterClauseParamsForUserCart(java.util.Map<String, String> paramsMap, Query query)
	{
	}
	public JsonObject retrieveUserCartList(java.util.Map<String, String> paramsMap)
	{
		List<Integer> userCartIdsList = new ArrayList<>();
		if(paramsMap.containsKey("userCartId"))
		{
			int userCartId = Integer.parseInt(paramsMap.get("userCartId"));
			if(userCartId>0)
			{
				userCartIdsList.add(userCartId);
			}
		}
		return retrieveUserCartList(paramsMap, userCartIdsList);
	}
	public JsonObject retrieveUserCartList(java.util.Map<String, String> paramsMap, List<Integer> userCartIdsList)
	{
		JsonObject dataObject = new JsonObject();
		try
		{
			...
			if(paramsMap.containsKey("txnStatus"))
			{
				txnStatus = paramsMap.get("txnStatus");
						...
						String cartCreationTime = paramsMap.get("cartCreationTime");
			if(cartCreationTime==null)
			{
				cartCreationTime = "";
			}
			String cartCheckoutTime = paramsMap.get("cartCheckoutTime");
			if(cartCheckoutTime==null)
			{
				cartCheckoutTime = "";
			}
String hql = "FROM UserCart where 1 = 1 ";
			String orderByClauseText = "  ";
			String userCartIdFilterClass = "";
			if (userCartIdsList != null && userCartIdsList.size() > 0)
			{
				userCartIdFilterClass = " AND userCartId in (:idsList) ";
			}
						String cartCreationTimeFilterClass = "";
			if (cartCreationTime.length() > 0)
			{	
				
				
				cartCreationTimeFilterClass = " AND cartCreationTime = :cartCreationTime ";		
			}
			String cartCheckoutTimeFilterClass = "";
			if (cartCheckoutTime.length() > 0)
			{	
				
				
				cartCheckoutTimeFilterClass = " AND cartCheckoutTime = :cartCheckoutTime ";		
			}
String txnStatusFilterClass = "";
			...
			String parentFilterClause  = getParentFilterClauseForUserCart(paramsMap);	        
			...
			String attributesFilterClause = 
					userCartIdFilterClass +
										cartCreationTimeFilterClass +
					cartCheckoutTimeFilterClass +
			...
			if (userCartIdsList != null && userCartIdsList.size() > 0)
			{
				query.setParameterList("idsList", userCartIdsList);
			}
						if (cartCreationTime.length() > 0)
			{	
				
				
				query.setParameter("cartCreationTime", CommonUtil.getDBFormattedTimeFrom12HoursFormatWithSeconds(cartCreationTime));		
			}
			if (cartCheckoutTime.length() > 0)
			{	
				
				
				query.setParameter("cartCheckoutTime", CommonUtil.getDBFormattedTimeFrom12HoursFormatWithSeconds(cartCheckoutTime));		
			}
if(lookupDisplayPrefix.length() > 0)
			{
	    	...
	    	setParentFilterClauseParamsForUserCart(paramsMap, query);
			...
			JsonArray userCartList = new JsonArray();
			for (Iterator it = resultsList.iterator(); it.hasNext();)
			{
				UserCart userCart = (UserCart) it.next();
				JsonObject userCartDataObject = userCart.getDataObject(getDBSession());
				userCartDataObject.addProperty("nextAction", getActionForCurrentStatus(userCart.getVwTxnStatus()));
				userCartList.add(userCartDataObject);
				doAfterSearchResultRowAddedImpl(userCartDataObject, queryParamsMap);
			}
			if (noOfRecordsAlreadyFetched == 0)
			{
				String countHql = "select count(*)  from UserCart where 1 = 1 ";
				countHql +=  whereClauseText;
				Query countQuery = getDBSession().createQuery(countHql);
				if (userCartIdsList != null && userCartIdsList.size() > 0)
				{
					countQuery.setParameterList("idsList", userCartIdsList);
				}
								if (cartCreationTime.length() > 0)
				{
					...
					countQuery.setParameter("cartCreationTime", CommonUtil.getDBFormattedTimeFrom12HoursFormatWithSeconds(cartCreationTime));
				...
				if (cartCheckoutTime.length() > 0)
				{
					...
					countQuery.setParameter("cartCheckoutTime", CommonUtil.getDBFormattedTimeFrom12HoursFormatWithSeconds(cartCheckoutTime));
		    	...
		    	setParentFilterClauseParamsForUserCart(paramsMap, countQuery);
		        for(int i =0; i< additionalParamsList.size(); i++)
		        {
		        	JsonObject additonalParamInfo = additionalParamsList.get(i).getAsJsonObject();
		        	String additionalParameterName = additonalParamInfo.get("parameterName").getAsString();
			...
			dataObject.add("userCartList",   userCartList);
			dataObject.addProperty("success", 1);
			return dataObject;
		}
		catch (Exception e)
	...
	public JsonObject getDashboardGraphsData()
	{
		...
		String hql = " SELECT concat(month(E.vwCreationDate), '/', year(E.vwCreationDate)), count(*) "
				+ "   from UserCart E GROUP BY year(E.vwCreationDate), month(E.vwCreationDate) ";
		Query query = getDBSession().createQuery(hql);
		List resultsList = query.list();
		...
		hql = " SELECT E.vwTxnStatus, count(*) "
				+ "   from UserCart E GROUP BY E.vwTxnStatus ";
	...
	public JsonObject retrieveDependentUserCartList(java.util.Map<String, String> paramsMap)
	{
		return retrieveUserCartList(paramsMap);
	}
	public UserCart getUserCartByLegacyRecordId(Session session, Integer legacyRecordId)
	{
		String hql = "from UserCart where legacyRecordId = :legacyRecordId ";
		Query query = getDBSession().createQuery(hql);
	...
	private void callBusinessUpdateRules()
	{
		...
		UserCart userCart = (UserCart)getManagedBean();
		JsonObject userCartDataObject = userCart.getDataObject(getDBSession());copyUserCartFromJson(userCart, userCartDataObject);
		setManagedBean(userCart);
		//setUpdatedBean(); 
	}	
	// Begin Test Data
	public void setTestData()
	{
		debugCode("In setTestData UserCartContollerBase");
			UserCart currentBean = (UserCart)getManagedBean();
		int iFieldLength = 0;
		int iFieldCounter = 1;
		String sFieldLength;
		String sStringTestData;
				
		currentBean.setCartCreationTime(new Time(new Date().getTime()));
		
		currentBean.setCartCheckoutTime(new Time(new Date().getTime()));

		setManagedBean(currentBean);
		debugCode("Out setTestData UserCartContollerBase");
	}
	// end Test Data
	public void copyUserCartFromJson(UserCart userCart, JsonObject userCartDataObject)
	{
		copyUserCartFromJson(userCart, userCartDataObject, false);
	}
	public void copyUserCartFromJson(UserCart userCart, JsonObject userCartDataObject, boolean excludePasswordFields)
	{	
				
		if(userCartDataObject.has("cartCreationTime"))
		{
			String cartCreationTimeString = userCartDataObject.get("cartCreationTime").getAsString();
			if(cartCreationTimeString.length() > 0)
			{
				try
				{
		...
		if(userCartDataObject.has("cartCheckoutTime"))
		{
			String cartCheckoutTimeString = userCartDataObject.get("cartCheckoutTime").getAsString();
			if(cartCheckoutTimeString.length() > 0)
			{
				try
				{
	...
	public JsonObject createUserCart(JsonObject userCartDataObject) throws Exception
	{
		return createUserCart(userCartDataObject, -1, null);
	}
	public void setLoginBasedValues(JsonObject paramsInfoObj, JsonObject userCartDataObject)
	{
		JsonObject userSessionInfo = getUserSessionInfo();			
		int userId = userSessionInfo.get("userId").getAsInt();
		String loginEntityType = userSessionInfo.get("loginEntityType").getAsString();
	...
	public JsonObject createUserCart(JsonObject userCartDataObject, int createdBy, JsonObject paramsInfoObj) throws Exception
	{
		UserCart userCart = new UserCart();
		setLoginBasedValues(paramsInfoObj, userCartDataObject);
		Session session = getDBSession();				
		copyUserCartFromJson(userCart, userCartDataObject);
		if(userCartDataObject.has("legacyRecordId"))
		{
			userCart.setLegacyRecordId(userCartDataObject.get("legacyRecordId").getAsInt());
		}
				userCart.setVwCreatedBy(createdBy);
		userCart.setVwCreationDate(new Date());
		setPrivateManagedBean(userCart);
		setManagedBean("UserCartBean", userCart);
		if (getHasTransactionFailed() == false)
		{
			create(paramsInfoObj);
		}
		userCart = (UserCart) getManagedBean();
		JsonObject dataObject = new JsonObject();
		if (getHasTransactionFailed() == true)
		{
		...
		else
		{
			...
			dataObject.addProperty("userCartId", userCart.getUserCartId());
			JsonObject userCartObj = userCart.getDataObject(getDBSession());
			userCartObj.addProperty("nextAction", getActionForCurrentStatus(userCart.getVwTxnStatus()));
			dataObject.add("userCartDataObject", userCartObj);
			dataObject.addProperty("success", 1);
		}
		return dataObject; 
	}
	public JsonObject updateUserCart(JsonObject userCartDataObject) throws Exception
	{
		Session session = getDBSession();
		JsonObject dataObject = new JsonObject();
		Integer userCartId = userCartDataObject.get("userCartId").getAsInt();
		JsonObject searchParams = new JsonObject();
		searchParams.addProperty("userCartId", userCartId);
		JsonObject responseData = retrieveUserCart(searchParams, new JsonObject());
		if(!responseData.has("success") || responseData.get("success").getAsInt() !=1)
		{
			dataObject.addProperty("alert", "Your request could not be processed as, selected 'UserCart' could not be found!!");			
			dataObject.addProperty("success", 0);			
			return dataObject;
		}
		...
		if(userCart.getIsRequestUnderProcesss() == 1)
		{
		...
		removeUpdateDisabledFromUserCart(userCartDataObject);
		boolean excludePasswordFields = true;
		copyUserCartFromJson(userCart, userCartDataObject, excludePasswordFields);setPrivateManagedBean(userCart);
		setManagedBean("UserCartBean", userCart);
		...
		userCart.setVwTxnStatus("MODIFIED");if (getHasTransactionFailed() == false)
		{
			update();
		}
		if (getHasTransactionFailed() == true)
	...
	public void removeUpdateDisabledFromUserCart(JsonObject userCartDataObject) throws Exception
	{
	}
	public JsonObject deleteUserCart(JsonObject userCartDataObject) throws Exception
	{
		Session session = getDBSession();
		JsonObject dataObject = new JsonObject();
		Integer userCartId = userCartDataObject.get("userCartId").getAsInt();
		...
		UserCart userCart = (UserCart) session.get(UserCart.class, userCartId);setPrivateManagedBean(userCart);
		setManagedBean("UserCart", userCart);
		if (getHasTransactionFailed() == false)
		{
			delete();
		}
	...
	public JsonObject fetchUserCartDefaultData(int obj, JsonObject initParams) throws Exception
	{
		Session session = getDBSession();
		UserCart userCart = new UserCart();
		JsonObject dataObject = new JsonObject();
		try
		{
			setPrivateManagedBean(userCart);
			setManagedBean("UserCart", userCart);
			doAfterUserCartLoaded(obj, initParams);
			if (getHasTransactionFailed() == false)
			{
				dataObject.add("userCart", userCart.getDataObject(getDBSession()));
				dataObject.addProperty("success", 1);
				return dataObject;
			}
			else
	...
	public JsonObject fetchUserCartTestData(int obj, JsonObject userCartDataObject) throws Exception
	{
		Session session = getDBSession();
		UserCart userCart = new UserCart();
		JsonObject dataObject = new JsonObject();
		try
		{
			...
			setPrivateManagedBean(userCart);
			setManagedBean("UserCart", userCart);
			doAfterUserCartLoaded(obj, null);
			setTestData();			
			if (getHasTransactionFailed() == false)
			{
				dataObject.add("userCart", userCart.getDataObject(getDBSession()));
	...
	public JsonObject lookupRowSelectedForUserCart(JsonObject userCartDataObject) throws Exception
	{
		UserCart userCart = new UserCart();
		Session session = getDBSession();
		JsonObject dataObject = new JsonObject();
		try
		{
			copyUserCartFromJson(userCart, userCartDataObject);	String attributeName = userCartDataObject.get("attributeName").getAsString();
			Integer attributeId = userCartDataObject.get("attributeId").getAsInt();
			setPrivateManagedBean(userCart);
			setManagedBean("UserCart", userCart);
	...
	public JsonObject selectOptionChangedForUserCart(JsonObject userCartDataObject) throws Exception
	{
		UserCart userCart = new UserCart();
		Session session = getDBSession();
		JsonObject dataObject = new JsonObject();
		...
		{
			copyUserCartFromJson(userCart, userCartDataObject);	
			String attributeName = userCartDataObject.get("attributeName").getAsString();
			setPrivateManagedBean(userCart);
			setManagedBean("UserCart", userCart);
	...
	public JsonObject doExecuteCustomAPI(JsonObject userCartDataObject) throws Exception
	{
		Session session = getDBSession();
		JsonObject dataObject = new JsonObject();
		try
		{
			Integer userCartId = userCartDataObject.get("userCartId").getAsInt();
			String customEventName = userCartDataObject.get("customEventName").getAsString();
			UserCart userCart = (UserCart) session.get(UserCart.class, userCartId);
			setPrivateManagedBean(userCart);
			setManagedBean("UserCartBean", userCart);
			beginTransaction();
	...
	public JsonObject executeAuthorisationOnUserCart(JsonObject userCartDataObject) throws Exception
	{
		Session session = getDBSession();
		JsonObject dataObject = new JsonObject();
		try
		{
			Integer userCartId = userCartDataObject.get("userCartId").getAsInt();
			String currentStatus = userCartDataObject.get("currentStatus").getAsString();
			String currentAction = "";
			if(userCartDataObject.has("currentAction"))
			{
				currentAction = userCartDataObject.get("currentAction").getAsString();
			}
			UserCart userCart = (UserCart) session.get(UserCart.class, userCartId);
			setPrivateManagedBean(userCart);
			setManagedBean("UserCartBean", userCart);
			...
			{
				executeAction(userCart, "ClassInfoBean", currentStatus, currentAction);
			}
			else
			{
				executeAction(userCart, "ClassInfoBean", currentStatus);
			}
//			executeAction(userCart, "UserCartBean", currentStatus);
			if (getHasTransactionFailed() == false)
			{
				JsonObject updateduserCartDataObject = userCart.getDataObject(getDBSession());
				updateduserCartDataObject.addProperty("nextAction", getActionForCurrentStatus(userCart.getVwTxnStatus()));
				dataObject.add("userCart", updateduserCartDataObject);
				if(!currentAction.equalsIgnoreCase(ACTION_REJECT))
				{
	...
	{
		UserCart userCart = (UserCart) getManagedBean();
		String currentStatus = userCart.getVwTxnStatus();
	...
	public JsonObject downloadUserCartData() throws Exception
	{
		return downloadUserCartData(null);
	}
	public JsonObject downloadUserCartData(HSSFWorkbook workbook) throws Exception
	
	{
		...
		try
		{
			...
			workbook.setSheetName(workbook.getSheetIndex(sheet), "UserCart");
			...
			for (Iterator it = resultsList.iterator(); it.hasNext();)
			{
				...
				Integer userCartId = userCart.getUserCartId();
				cell = row.createCell(dataCellCount++);
				cell.setCellStyle(dataStyle);
				...
				Time cartCreationTime = userCart.getCartCreationTime();
				if(cartCreationTime!=null)
				{
					cell.setCellValue(CommonUtil.getTimeInRegular12HoursFormatWithSeconds(cartCreationTime));
				}
				cell = row.createCell(dataCellCount++);
				cell.setCellStyle(dataStyle);		
				
				Time cartCheckoutTime = userCart.getCartCheckoutTime();
				if(cartCheckoutTime!=null)
				{
					cell.setCellValue(CommonUtil.getTimeInRegular12HoursFormatWithSeconds(cartCheckoutTime));
				}
			...
			if(saveWorkbook == true)
			{
				...
				String fileName = CommonUtil.getFileNameByCurrentTime() + "_" + "UserCart.xls";
				String savedFileName = filePath + CommonUtil.getFileNameByCurrentTime() + "_" + "UserCart.xls";
	...
	public JsonObject uploadUserCartData(JsonObject userCartDataObject) throws Exception
	{
		JsonObject dataObject = new JsonObject();
		Integer fileId = userCartDataObject.get("fileId").getAsInt();
		String inputFilesZip = userCartDataObject.get("inputFilesZip").getAsString();
		...
		if(inputFilesZip.length() > 0)
		{
	        ...
	        if(isExtracted != 1)
	        {
	    		dataObject.addProperty("alert", "UserCart Data could not be uploaded as input files zip could not be extracted.");
	    		dataObject.addProperty("success", 0);
	    		return dataObject;
	        }
	        inputFilesPath = extractedZipFilePath;
		}
		userCartDataObject.addProperty("inputFilesPath", inputFilesPath);
		JsonObject responseData = uploadUserCartData(workbook, userCartDataObject);
		...
		workbook.write(out);
		out.close();
		dataObject.addProperty("alert", "UserCart Data uploaded successfully.");
		dataObject.addProperty("success", 1);
		dataObject.addProperty("fileId", fileId);
		return dataObject;
	}
	...
	public JsonObject uploadUserCartData(HSSFWorkbook workbook, JsonObject userCartDataObject) throws Exception
	{
		Session session = getDBSession();
		JsonObject dataObject = new JsonObject();
		try
		{
			...
			Integer fileId = -1;
			Integer areSourceDestinationSame = userCartDataObject.get("areSourceDestinationSame").getAsInt();
			String inputFilesPath = userCartDataObject.get("inputFilesPath").getAsString();
			if(workbook == null)
			{
				fileId = userCartDataObject.get("fileId").getAsInt();
	...
	public JsonObject processExcelSheetData(HSSFWorkbook workbook, HSSFSheet sheet, HSSFCellStyle successCellStyle, HSSFCellStyle errorCellStyle, Integer areSourceDestinationSame, String inputFilesPath) throws Exception
	{
		...
		try
		{
			...
			JsonObject userCart = new JsonObject();
			int totalRetryCount = 0;
			while (currentRowPosition < totalDefinedRowsInSheet)
			{
				String rowFirstCellValue = "";
				String dependentEntityName = "";
				JsonObject userCartListObj = fetchData(workbook, sheet, totalDefinedRowsInSheet, batchsize, rowColumnIndexObject, cellIndexParameterMap, areSourceDestinationSame, inputFilesPath);
				JsonArray userCartList = userCartListObj.get("userCartList").getAsJsonArray();
				currentRowPosition = rowColumnIndexObject.get("currentRowPosition").getAsInt();
				JsonObject responseData = uploadUserCartList(userCartList);
				if (responseData == null || !responseData.has("success") || responseData.get("success").getAsInt() != 1)
				{
				...
				JsonArray dataObjectsListToRetry = UploadDownloadUtil.getDataToRetry(userCartList);
				dataListToRetry.addAll(dataObjectsListToRetry);
				updateStatusMsg(userCartList, sheet, successCellStyle, errorCellStyle, statusCellIndex);				
			}
			dataObject.addProperty("success", 1);
			dataObject.add("dataListToRetry", dataListToRetry);			
	...
	public JsonObject fetchData(HSSFWorkbook workbook, HSSFSheet sheet, int totalDefinedRowsInSheet, int batchsize, JsonObject rowColumnIndexObject, HashMap<Integer, String> cellIndexParameterMap, Integer areSourceDestinationSame,String inputFilesPath) throws Exception	
	{
		...
		{			
			...
			while (currentRowPosition < totalDefinedRowsInSheet)
			{
				...
				JsonObject userCartUploadObj	= new JsonObject();
				userCartUploadObj.addProperty("dataRowIndex", currentRowPosition);		    
				row = sheet.getRow(currentRowPosition++);
				int cellIndex = entityDataCellIndex+1;
				try
				{
					userCart = new JsonObject();
					for (int i = 0; i < cellIndexParameterMap.size(); i++)
					{
						String parameterName = cellIndexParameterMap.get(cellIndex);
						if (parameterName.equalsIgnoreCase("userCartId"))
						{
							String userCartId = row.getCell(cellIndex++).getStringCellValue();
							if(userCartId != null && userCartId.trim().length() > 0)
							{
								try
								{
									Integer iUserCartId = Integer.parseInt(userCartId);
									if(areSourceDestinationSame == 1)
									{
										UserCart userCartObj = (UserCart)session.get(UserCart.class, iUserCartId);
										if(userCartObj != null)
										{ 
											userCart.addProperty("userCartId", iUserCartId);
										}
										else
										{
											userCartUploadObj.addProperty("isDataFetched", 0);
											userCartUploadObj.addProperty("msg", "This UserCart could not be uploaded as there appears to be some problem with the data(UserCart Id is not exist). ");
											continue;
										}
									}
									else
									{
										UserCart userCartObj = getUserCartByLegacyRecordId(session, iUserCartId);
										if(userCartObj != null)
										{ 
											userCart.addProperty("userCartId", userCartObj.getUserCartId());
										}
										userCart.addProperty("legacyRecordId", iUserCartId);
									}
								}
								catch (Exception e)
								{
									writeToLog(CommonUtil.getStackTrace(e));
									userCartUploadObj.addProperty("isDataFetched", 0);
									userCartUploadObj.addProperty("msg", "This UserCart could not be uploaded as there appears to be some problem with the data(UserCart Id). ");
									continue;
								}
							}
						}
						else
						{
							...
							userCart.addProperty(parameterName, parameterValue);
						}
					}
					userCartUploadObj.add("dataObject", userCart);		    
					userCartList.add(userCartUploadObj);
					pendingRecordsCount++;
	...
	public int  areSomeRecordsUploaded(JsonObject previousRetryCountMap, JsonObject retryCountMap) throws Exception
	{
		...
		if(previousRetryCountMap.has("UserCart") && previousRetryCountMap.get("UserCart").isJsonNull()==false)
		{
			previousRetryCount = previousRetryCountMap.get("UserCart").getAsInt();
		}
		if(retryCountMap.has("UserCart") && retryCountMap.get("UserCart").isJsonNull()==false)
		{
			retryCount = retryCountMap.get("UserCart").getAsInt();
		}
	...
	public void updateRetryCountMapForUserCartList(JsonArray userCartList, JsonObject retryCountMap) throws Exception
	{
		int retryCount = 0;
		for (int i= 0; i < userCartList.size(); i++)
		{
			...
			JsonObject userCartDataObject = userCartList.get(i).getAsJsonObject();
			JsonObject userCart = userCartDataObject.get("dataObject").getAsJsonObject();
			if(userCartDataObject.has("retryUpload") && userCartDataObject.get("retryUpload").isJsonNull()==false)
			{
				retryUpload = userCartDataObject.get("retryUpload").getAsInt();
			}
			if(userCartDataObject.has("retryChildEntitiesUpload") && userCartDataObject.get("retryChildEntitiesUpload").isJsonNull()==false)
			{
				retryChildEntitiesUpload = userCartDataObject.get("retryChildEntitiesUpload").getAsInt();
			}
			if(userCartDataObject.has("partialUploadUnderProcess") && userCartDataObject.get("partialUploadUnderProcess").isJsonNull()==false)
			{
				partialUploadUnderProcess = userCartDataObject.get("partialUploadUnderProcess").getAsInt();
			}
	    ...
	    retryCountMap.addProperty("UserCart", retryCount);
	}
	public JsonObject uploadUserCartList(JsonArray userCartList) throws Exception
	{
		JsonObject dataObject = new JsonObject();
		try
		{
			JsonObject responseData;
			responseData = uploadData(userCartList);
			if (responseData == null || !responseData.has("success") || responseData.get("success").getAsInt() != 1)
			{
				dataObject.addProperty("alert", responseData.get("alert").getAsString());
				dataObject.addProperty("success", 0);
	...
	public JsonObject updateStatusMsg(JsonArray userCartList, HSSFSheet sheet, HSSFCellStyle successCellStyle, HSSFCellStyle errorCellStyle, int statusCellIndex) throws Exception
	{
		JsonObject dataObject = new JsonObject();
		try
		{
			...
			for (int i= 0; i < userCartList.size(); i++)
			{
				JsonObject userCartDataObject = userCartList.get(i).getAsJsonObject();
				JsonObject userCart = userCartDataObject.get("dataObject").getAsJsonObject();
				int isSuccessfullyUploaded = userCartDataObject.get("isSuccessfullyUploaded").getAsInt();
				int dataRowIndex = userCartDataObject.get("dataRowIndex").getAsInt();
				String errorMessage = userCartDataObject.get("errorMessage").getAsString();
	...
	public JsonObject uploadData(JsonArray userCartList) throws Exception
	
	{
		...
		try
		{
			for (int i =0; i < userCartList.size(); i++)
			{
				...
				JsonObject userCartDataObject = userCartList.get(i).getAsJsonObject();
				JsonObject userCart = userCartDataObject.get("dataObject").getAsJsonObject();
				userCartDataObject.addProperty("retryUpload", 0);
				userCartDataObject.addProperty("retryChildEntitiesUpload", 0);
				userCartDataObject.addProperty("partialUploadUnderProcess", 0);
				com.shoppingcart.controller.forms.impl.UserCartControllerImpl userCartImplObj = new com.shoppingcart.controller.forms.impl.UserCartControllerImpl(session, getUserSessionInfo());				
				int areAllLookupsFound = userCartImplObj.getEntityInfoUpdatedWithLookupIds(session, userCart, retValObject);
				if(areAllLookupsFound!=1)
				{
					userCartDataObject.addProperty("retryUpload", 1);
					userCartDataObject.addProperty("errorMessage", "Selected lookup entities information not available while uploading this row.");				
					userCartDataObject.addProperty("isSuccessfullyUploaded", 0);				
					retryCount++;
					continue;
				}
				...
				{
					userCartDataObject.addProperty("partialUploadUnderProcess", partialUploadUnderProcess);					
				}
				...
				if(keyColumnsCount > 0 && !userCart.has("userCartId"))
				{
					userCart.addProperty("attributeNamePrefix", "");
					
					userCart.addProperty("attributeNamePrefix", "");
					JsonObject responseData = userCartImplObj.getUserCartByLookupFields(session,  userCart);
					if (responseData == null || !responseData.has("success") || responseData.get("success").getAsInt() != 1)
					{
					...
					{
						JsonObject userCartObject = responseData.get("userCartDataObject").getAsJsonObject();
						userCartId = userCartObject.get("userCartId").getAsInt();
						userCart.addProperty("userCartId", userCartId);
						updateEntity = true;
					}
				}
				else if(userCart.has("userCartId"))
				{
					updateEntity = true;
				}
				...
				{
					responseData = userCartImplObj.createUserCart(userCart);
				}
				else
				{
					responseData = userCartImplObj.updateUserCart(userCart);
				}
				userCartDataObject.addProperty("isSuccessfullyUploaded", 1);				
				Transaction tx = session.getTransaction();
				if (responseData == null || !responseData.has("success") || responseData.get("success").getAsInt() != 1)
				{
					userCartId =-1;
					userCartDataObject.addProperty("errorMessage", responseData.get("alert").getAsString());				
					userCartDataObject.addProperty("isSuccessfullyUploaded", 0);
					if (tx.isActive())
					{
						tx.rollback();						
				...
				userCartId = responseData.get("userCartId").getAsInt();
				userCartDataObject.addProperty("errorMessage", responseData.get("alert").getAsString());				
	...
	public int getEntityInfoUpdatedWithLookupIds(Session session, JsonObject userCart, JsonObject retValObject)
	{
		JsonObject requestParams = new JsonObject();
		JsonObject dataObject = new JsonObject();
		int hasParamsForLookup = 0;return 1;		
	}
	public JsonObject getUserCartByLookupFields(Session session, JsonObject requestParams)
	{
		JsonObject dataObject = new JsonObject();
		try
		{String hql = "From UserCart where 1 = 1  ";
			...
			if(resultsCount != 1)
			{
				dataObject.addProperty("alert", "Your request could not be processed as UserCart could not be retrieved");
				dataObject.addProperty("success", 0);
			...
			for (Iterator it = resultsList.iterator(); it.hasNext();)
			{
				...
				dataObject.add("userCartDataObject", userCartDataObject);
				dataObject.addProperty("success", 1);
				return dataObject;
			}
		}
	...
	public static JsonObject getQueryParamsForLookupSearch(JsonObject searchObject, String attributeNamePrefix)
	{
		...
		dataObject.addProperty("alert", "Your request could not be processed as lookup information for 'UserCart' could not be retrieved");
		dataObject.addProperty("success", 0);
		return dataObject;
	}
	...
	public JsonObject getAPIData(Session session, String apiName, JsonObject paramsInfo)
	{
		...
		else if(apiName.equals("getUserCartPropertyValue"))
			{
				JsonObject inputDataObject = getUserCartPropertyValue(session, requestParametersInfo);
				inputDataObject.addProperty("success", 1);
				return inputDataObject;
			}
			else if(apiName.equals("getUserCart"))
			{
				JsonObject inputDataObject = getUserCart(session, requestParametersInfo);
				inputDataObject.addProperty("success", 1);
				return inputDataObject;
			}		dataObject.addProperty("success", 0);
			return dataObject;
	...
	public int updateAPIStatus(Session session, int requestId, JsonObject retValObject)
	{
		...
		try
		{
			...
			else if (apiName.equals("doBeforeTransactionApprovedForUserCart"))
			{
				isAPIExecuted = 1;
				dataObject = updateAPIStatus(session, requestReceived);
			}
			else if (apiName.equals("doBeforeTransactionRolledbackForUserCart"))
			{
				isAPIExecuted = 1;
				dataObject = updateAPIStatus(session, requestReceived);
			}
	...
	public JsonObject updateAPIStatus(Session session, RequestReceived requestReceived)
	{
		...
		try
		{
			...
			Integer userCartId = requestReceivedParametersInfo.get("userCartId").getAsInt();
			UserCart userCart = (UserCart) session.get(UserCart.class, userCartId);
			userCart.setIsRequestUnderProcesss(0);
	...
	public JsonObject getUserCartPropertyValue(Session session, JsonObject paramsInfo)
	{
		JsonObject dataObject = new JsonObject();
		JsonObject inputForGetAPI = paramsInfo.get("inputForGetAPI").getAsJsonObject();
		Integer userCartId = inputForGetAPI.get("userCartId").getAsInt();
		String popertyNameEL = inputForGetAPI.get("popertyNameEL").getAsString();
		UserCart userCart = (UserCart) session.get(UserCart.class, userCartId);
		userCart.getPropertyValue(session, popertyNameEL, dataObject);
		return dataObject;
	}
	public JsonObject getUserCart(Session session, JsonObject paramsInfo)
	{
		JsonObject dataObject = new JsonObject();
		JsonObject inputForGetAPI = paramsInfo.get("inputForGetAPI").getAsJsonObject();
		Integer userCartId = inputForGetAPI.get("userCartId").getAsInt();
		Map<String, String> paramsMap = new HashMap<String, String>();
		paramsMap.put("userCartId", String.valueOf(userCartId));
		JsonObject userCartListObject = retrieveUserCartList(paramsMap);
		if(userCartListObject!=null && userCartListObject.has("success") && userCartListObject.get("success").getAsInt()==1)
		{
			JsonArray userCartList = userCartListObject.get("userCartList").getAsJsonArray();
	...
	public abstract JsonObject doExecuteAPIRequestImpl(String apiName, JsonObject userCartDataObject, UserCart userCart);
	...
	public abstract void doAfterLookupRowSelectedImpl(UserCart userCart, String attributeName);
	public abstract void doAfterSelectOptionChangedImpl(UserCart userCart, String attributeName);
	...
	public abstract void doAfterEntityLoadedImpl(UserCart userCart, JsonObject initParamsInfo);
	...
	public abstract void doUpdateUserCart(Session session);
}
