...
import com.shoppingcart.util.layout.QueryBuilderReporting;
...
public class AjaxRequestHandler
{
	...
	public JsonObject handleAjaxRequest(HttpServletRequest request, HttpServletResponse response, JsonObject userSessionInfo, JsonObject requestInfo, Session masterSession, Session organisationSession)
	{
		...
		try
		{
			...
			// Handling CheckOut API

			else if ("/customCheckOut".equalsIgnoreCase(requestURI)) {
				dataObject = entityRequestHandler.handleCustomAPIAjaxRequest(userSessionInfo, requestInfo,
						masterSession, organisationSession);
			}
            
			else if ("/UserCartCheckOut".equalsIgnoreCase(requestURI)) {
				dataObject = entityRequestHandler.handleCustomAPIAjaxRequest(userSessionInfo, requestInfo,
						masterSession, organisationSession);
			}
			// Handling CheckOut API
